...
- Run a script inside the chroot:
  ...
  .. code-block:: bash
     ...
     cp /path/to/your/script.sh $ARM64_SYSROOT/path/to/your/script.sh
...
.. note::
   ...
   #. The Docker container itself (**AMD64** architecture or **ARM64** architecture, depending on the host machine)
   #. The chroot environment (**ARM64** architecture)

   When using ``arm64-chroot <command>``, the command runs inside the ARM64 chroot environment. File paths from the Docker container are not directly accessible. You must copy files into the sysroot (``$ARM64_SYSROOT``) before referencing them inside the chroot.
...
#. Syncs the workspace source code into the sysroot (``$ARM64_SYSROOT/home/ubuntu/ros2_ws``).
