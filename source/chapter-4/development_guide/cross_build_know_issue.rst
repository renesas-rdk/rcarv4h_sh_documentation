...
.. code-block:: text
   ...
   CMake Error at /opt/arm64_sysroot/opt/ros/jazzy/lib/aarch64-linux-gnu/cmake/pinocchio/pinocchioTargets.cmake:146 (message):
...
- reads the base patch rules from ``/home/ubuntu/toolchains/sysroot-fix.yaml``,
- reads your user-defined rules from ``/home/ubuntu/toolchains/sysroot-fix-append.yaml``,
- searches known CMake export files inside the sysroot, and
- replaces known hardcoded paths with relocatable paths.

The base rule file ``sysroot-fix.yaml`` is updated automatically when the container starts, so do **not** edit it — your changes would be overwritten.
User-added rules belong in ``sysroot-fix-append.yaml``, which is gitignored and seeded from ``sysroot-fix-append.template.yaml`` on container start.
...
In that case, you need to add a new rule to ``sysroot-fix-append.yaml`` manually.
...
.. code-block:: bash
   ...
   grep -RIn "/opt/ros/${ROS_DISTRO}" ${ARM64_SYSROOT}/opt/ros/${ROS_DISTRO}/lib/aarch64-linux-gnu/cmake/pinocchio

If the package is not under that path, search more broadly:

.. code-block:: bash

   grep -RIn "/opt/ros/${ROS_DISTRO}" ${ARM64_SYSROOT}/opt/ros/${ROS_DISTRO}

or:

.. code-block:: bash

   grep -RIn "/usr/lib/aarch64-linux-gnu" ${ARM64_SYSROOT}/usr/lib ${ARM64_SYSROOT}/opt/ros/${ROS_DISTRO}
...
Step 4: Add a new rule to ``sysroot-fix-append.yaml``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Open the user rule file:

.. code-block:: bash

   editor /home/ubuntu/toolchains/sysroot-fix-append.yaml

.. note::

   Do **not** add your rules to ``/home/ubuntu/toolchains/sysroot-fix.yaml``.
   That file contains the shipped base rules and is overwritten by the container auto-update.
   User-added rules must go into ``sysroot-fix-append.yaml``.
...
.. code-block:: yaml
   ...
   <package_name>:
     - file: "<path relative to ARM64_SYSROOT or absolute path with ${ARM64_SYSROOT}>"
...
.. note::
   ...
   The variables ``ARM64_SYSROOT`` and ``ROS_DISTRO`` are already provided by the ``sysroot-fix`` script.
...
.. code-block:: text
   ...
   [sysroot-fix] Package: pinocchio
     - PATCH: /opt/arm64_sysroot/opt/ros/jazzy/lib/aarch64-linux-gnu/cmake/pinocchio/pinocchioTargets-none.cmake
       find:    /opt/ros/jazzy
       replace: ${_IMPORT_PREFIX}
...
.. code-block:: text
   ...
   backup: /opt/arm64_sysroot/.../some_file.cmake.bak.20260316-032342
...
You should add or adjust a rule in ``sysroot-fix-append.yaml`` when:
...
#. Add a matching rule to ``/home/ubuntu/toolchains/sysroot-fix-append.yaml``,
