# R-Car/V4H Sparrow Hawk (SH) User Manual

## Overview
This repository provides a ready-to-use User Manual for the **R-Car/V4H Sparrow Hawkt (SH)**.
...
[R-Car/V4H Sparrow Hawk User Manual (Online)](https://renesas-rdk.github.io/rcarv4h_sh_documentation/)

For offline use, download the generated PDF after each build at: `docs/pdf/RCar_V4H_Sparrow_Hawk_User_Manual.pdf`
...
1. Clone the repository and change the workspace to the cloned directory:
    ...
    ```bash
    git clone https://github.com/renesas-rdk/rcarv4h_sh_documentation.git
    cd rcarv4h_sh_documentation
    ```

2. Build the Docker image:

    ```bash
    docker build -f docker/image/Dockerfile --build-arg user=$(id -un) --build-arg uid=$(id -u) -t sh_documentation .
    ```
...
3. Create the container:
   ...
   Mount the current documentation repository inside the Docker container.
    ...
    ```bash
    docker run -dt --name sh_doc_build -v $(pwd):/tmp/doc_repository sh_documentation
    ```

4. Enter the Docker environment (for subsequent sessions):

    ```bash
    docker exec -it sh_doc_build bash
    ```
...
python3 scripts/convert.py \
  --output_filename RCar_V4H_Sparrow_Hawk_User_Manual.pdf \
  --release_version "${DOCS_VERSION#v}" \
  --rev_date "$(date '+%b.%d.%y')"
```

The output will be located at: `output/RCar_V4H_Sparrow_Hawk_User_Manual.pdf`
